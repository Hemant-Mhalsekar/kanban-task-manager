...
const COLUMN_STYLES = {
  todo:       { header: 'bg-slate-100',  dot: 'bg-slate-400',   label: 'To Do'       },
  inprogress: { header: 'bg-blue-50',    dot: 'bg-blue-500',    label: 'In Progress'  },
  done:       { header: 'bg-green-50',   dot: 'bg-green-500',   label: 'Done'         },
};
...
export default function Column({ columnId, cards, onAddCard, onDeleteCard, onUpdateCard }) {
  ...
  return (
    <div className="flex flex-col w-full min-w-0 rounded-2xl bg-gray-50 border border-gray-200 overflow-hidden">
      {/* Column header */}
      <div className={`flex items-center gap-2 px-4 py-3 ${style.header}`}>
        ...
        <span className="text-xs font-semibold text-gray-400 bg-white rounded-full px-2 py-0.5 border border-gray-200">
          {cards.length}
        </span>
      </div>
        ...
        {(provided, snapshot) => (
          ...
          >
            {cards.length === 0 && !showForm && !snapshot.isDraggingOver && (
              <p className="text-xs text-center text-gray-400 mt-4">No cards yet</p>
            )}
      ...
      {!showForm && (
        <div className="px-3 pb-3">
          <button
            ...
            className="w-full flex items-center gap-1.5 justify-center text-sm text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 border border-dashed border-gray-300 hover:border-indigo-300 rounded-xl py-2 transition-colors"
          >
